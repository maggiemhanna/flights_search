...
BASE_URL = "https://json-parser-service-874751466618.europe-west9.run.app"

def test_root_endpoint():
    """Test the root health-check endpoint."""
    response = requests.get(f"{BASE_URL}/")
    assert response.status_code == 200, f"Expected 200, got {response.status_code}. Details: {response.text}"
    assert response.json() == {"message": "JSON Parser Agent Service is running. Use the /run-json-parser endpoint via POST."}
...
def test_run_json_parser():
    ...
    for test in tests:        
        ...
        response = requests.post(f"{BASE_URL}/run-json-parser", json=payload)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("Running test_run_json_parser...")
        # Note: If this hits a real LLM, it might take a few seconds to return.
        test_run_json_parser()
        logger.info("✅ test_run_json_parser passed!")
