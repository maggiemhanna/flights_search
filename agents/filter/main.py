...
from agents.filter.agent import filter
from agents.filter.schema import FilterInput
...
APP_NAME = "filter"
ENV_FILE_PATH = Path("agents/filter/env.yaml")
...
# --- FastAPI Application ---
api = FastAPI(
    title="Filter Agent Service",
    description="API for running the Filter Agent."
)

@api.get("/", tags=["Health"])
async def root() -> Dict[str, str]:
    return {"message": "Filter Agent Service is running. Use the /run-filter endpoint via POST."}
...
async def execute_agent_run(user_input: FilterInput) -> List[Dict[str, Any]]:
    """Executes the filter agent runner and processes its responses."""
...
# --- FastAPI Endpoints ---
@api.post("/run-filter", response_model=Dict[str, Any], tags=["Agent"])
async def run_filter(user_input: FilterInput) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    uvicorn.run("main:api", host="127.0.0.1", port=8002, reload=False)