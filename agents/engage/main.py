...
from agents.engage.agent import engage
from agents.engage.schema import EngageInput
...
APP_NAME = "engage"
ENV_FILE_PATH = Path("agents/engage/env.yaml")
...
# --- FastAPI Application ---
api = FastAPI(
    title="Engage Agent Service",
    description="API for running the Engage Agent."
)

@api.get("/", tags=["Health"])
async def root() -> Dict[str, str]:
    return {"message": "Engage Agent Service is running. Use the /run-engage endpoint via POST."}
...
async def execute_agent_run(user_input: EngageInput) -> List[Dict[str, Any]]:
...
# --- FastAPI Endpoints ---
@api.post("/run-engage", response_model=Dict[str, Any], tags=["Agent"])
async def run_engage(user_input: EngageInput) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    uvicorn.run("main:api", host="127.0.0.1", port=8001, reload=False)